...
public:
	...
	vector<vector<int>> levelOrderBottom(TreeNode* root) {
		vector<vector<int>> upDownV = levelOrder(root);

		int start = 0, end = upDownV.size() - 1;

		while (start < end)
		{
			auto tmp = upDownV[start];
			upDownV[start] = upDownV[end];
			upDownV[end] = tmp;
...
int main()
{
	...
	auto retV = s.levelOrderBottom(root);
